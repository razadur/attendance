        ...
        <table class="table table-striped table-bordered">
            ...
            foreach($userdata as $rows)
            {
               ...
               $EndCount= count($data);
                ...
                $LateIn=0;//ok
                $LateOut=0;//ok
                if(trim($startCount)==0 && trim($EndCount)==0)
                {
                    $attandance=0;
                }
                else if(trim($startCount)==0 || trim($EndCount)==0)// if no data found in flaxible time
                {


                    if(trim($startCount)==0)
                    {
                ...
                if($totalUserData!=$i && trim($EndCount)==1 )
                {
               ...
               foreach($data as $r)
               {
                ...
                //$start_date = new DateTime("$EndTime");
                //$since_start = $start_date->diff(new DateTime("$startTime"));
                //$dutyTime= $since_start->h.':'.$since_start->i;
                $totalDutime1=$totalDutime1+floor($dutyTime);
                $totalDutime2=$totalDutime2+($dFraction/100)*60;
                $coutinus=0;
                if($firstTimePunchCount==true)
                {
                    $nextabsent=false;
